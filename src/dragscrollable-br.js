...
(function ($) {
  ...
  var left_top = function (event) {
    ...
    if (typeof event.clientX != "undefined") {
      ...
      console.error(
        "don't understand x and y for " + event.type + " event: " + str
      );
  ...
  $.fn.dragscrollable = function (options) {
    ...
    settings.dragstart = append_namespace(
      settings.dragstart,
      settings.namespace
    );
    settings.dragcontinue = append_namespace(
      settings.dragcontinue,
      settings.namespace
    );
    ...
    var dragscroll = {
      dragStartHandler: function (event) {
        ...
        handling_element.bind(
          settings.dragcontinue,
          event.data,
          dragscroll.dragContinueHandler
        );
        ...
        $.each(settings.dragend.split(" "), function (idx, val) {
          handling_element
            .get(0)
            .addEventListener(val, dragContinueHandlerProxy, true);
        });
      ...
      dragContinueHandler: function (event) {
        // User is dragging
      ...
      dragEndHandler: function (event) {
        ...
        $.each(settings.dragend.split(" "), function (idx, val) {
          handling_element
            .get(0)
            .removeEventListener(val, dragContinueHandlerProxy, true);
        });

        // How much did the mouse move total?
        var delta = {
          left: Math.abs(
            event.data.lastCoord.left - event.data.firstCoord.left
          ),
  ...
  $.fn.removedragscrollable = function (namespace) {
    if (typeof namespace == "undefined") namespace = ".ds";
    return this.each(function () {
