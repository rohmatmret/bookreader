...
BookReader.prototype.addChapter = function (
  ...
  const percentThrough = BookReader.util.cssPercentage(
    pageIndex,
    this.getNumLeafs() - 1
  );
  ...
  const mobileChapter = $(`<li></li>`)
    .append($(`<span class='BRTOCElementTitle'></span>`).text(title))
    .append($(`<span class='BRTOCElementPage'></span>`).text(pageStr));
  mobileChapter
    .addClass("BRtable-contents-el")
    ...
    $(`<div></div>`)
      ...
      .hover(
        (event) => {
          ...
          const boxSizeAdjust =
            parseInt(getComputedStyle(tooltip).paddingLeft) * 2;
          if (tooltipOffset.x - boxSizeAdjust < 0) {
            tooltip.style.setProperty(
              "transform",
              `translateX(-${targetOffset.left - boxSizeAdjust}px)`
            );
          }
  ...
  $.ajax({ url: fetchUrlByBookId, dataType: "jsonp" })
    .then((data) => {
      if (data && data.length > 0) {
        ...
        // try sourceid
        return $.ajax({
          url: `${baseURL}&source_records=ia:${this.bookId}`,
          dataType: "jsonp",
        });
      }
    })
...
BookReader.prototype.updateTOCState = function (currIndex, tocEntries) {
  ...
  $(".current-chapter").removeClass("current-chapter");
  const tocEntriesIndexed = tocEntries
    .filter((el) => el.pageIndex != undefined)
    .reverse();
  const currChapter =
    tocEntriesIndexed[
      tocEntriesIndexed.findIndex((el) => el.pageIndex <= currIndex)
    ];
