...
export class PageContainer {
  ...
  update({ dimensions = null, reduce = null }) {
    ...
    const nextBestLoadedReduce =
      !alreadyLoaded &&
      this.imageCache.getBestLoadedReduce(this.page.index, reduce);
    ...
    if (!alreadyLoaded) {
      ...
      backgroundLayers.push(
        `url("${this.loadingImage}") center/20px no-repeat`
      );
    }
 ...
 */
export function renderBoxesInPageContainerLayer(
  layerClass,
  boxes,
  page,
  containerEl
