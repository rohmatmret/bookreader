...
export class Mode1Up {
   ...
   **/
  physicalInchesToDisplayPixels(
    inches,
    reduce = this.realWorldReduce,
  ...
  *pagesWithBounds(
    reduce = this.realWorldReduce,
    pageSpacing = this.LEAF_SPACING_IN
  ) {
    let leafTop = 0;
    let leafBottom = 0;
    ...
    for (const page of this.book.pagesIterator({
      ...
      const height = this.physicalInchesToDisplayPixels(
        page.heightInches,
        reduce
      );
      leafBottom += height;
      ...
      leafTop +=
        height + this.physicalInchesToDisplayPixels(pageSpacing, reduce);
  ...
  static boundIntersection(bound1, bound2) {
    const intersect =
      bound1.bottom >= bound2.top && bound1.top <= bound2.bottom;
  ...
  *findIntersectingPages() {
    ...
    for (const { page, top, bottom } of this.pagesWithBounds()) {
      const intersection = Mode1Up.boundIntersection(
        { top, bottom },
        scrollRegion
      );
    ...
    if (pagesToDisplay.length) {
      ...
      for (const { page, top, bottom } of pagesToDisplay) {
        if (!this.br.displayedIndices.includes(page.index)) {
          ...
          const pageContainer = this.br
            ._createPageContainer(page.index)
            .update({
              dimensions: {
                width,
                height,
  ...
  jumpToIndex(index, pageX, pageY, noAnimate) {
    ...
    if (pageY) {
      ...
      offset =
        this.physicalInchesToDisplayPixels(
          (pageY / page.height) * page.heightInches
        ) -
        clientHeight / 2;
      leafTop += offset;
    } else {
    ...
    if (pageX) {
      ...
      offset =
        this.physicalInchesToDisplayPixels(
          (pageX / page.width) * page.widthInches
        ) -
        clientWidth / 2;
      leafLeft += offset;
    } else {
   ...
   */
  zoom(direction) {
    const nextReductionFactor = this.br.nextReduce(
      this.realWorldReduce,
      direction,
      this.br.onePage.reductionFactors
    );
  ...
  getAutofitWidth() {
    ...
    const availableWidth =
      this.$scrollContainer.prop("clientWidth") - 2 * widthPadding;

    const medianWidthInches = this.book.getMedianPageSizeInches().width;
    const medianPageWidth = this.physicalInchesToDisplayPixels(
      medianWidthInches,
      1
    );
  ...
  getAutofitHeight() {
    ...
    const availableHeight =
      this.$scrollContainer.innerHeight() - 2 * this.br.padding;
    const medianHeightInches = this.book.getMedianPageSizeInches().height;
    const medianPageHeight = this.physicalInchesToDisplayPixels(
      medianHeightInches,
      1
    );
  ...
  calculateReductionFactors() {
    ...
    if (autoWidthReduce < 1) {
      this.br.onePage.reductionFactors.find((r) => r.reduce == 1).autofit =
        "auto";
    } else {
      this.br.onePage.reductionFactors.find(
        (r) => r.autofit == "width"
      ).autofit = "auto";
    }
  }
  ...
  resizePageView() {
    ...
    if (oldScrollTop > 0) {
      ...
      const fudgeFactor =
        0.6 *
        this.physicalInchesToDisplayPixels(
          this.book.getPage(this.br.currentIndex()).heightInches
        );
  ...
  calculateViewDimensions(
    reduce = this.realWorldReduce,
    leafSpacing = this.LEAF_SPACING_IN
  ) {
    let width = 0;
    let height = 0;
    for (const { page, bottom } of this.pagesWithBounds(reduce, leafSpacing)) {
      const pageWidth = this.physicalInchesToDisplayPixels(
        page.widthInches,
        reduce
      );
  ...
  centerX(
    ...
    let centerX;
    if ($documentContainer.width() < $scrollContainer.prop("clientWidth")) {
      // fully shown
      centerX = $documentContainer.width();
    } else {
      centerX =
        $scrollContainer.scrollLeft() +
        $scrollContainer.prop("clientWidth") / 2;
    }
  ...
  centerY($scrollContainer = this.$scrollContainer) {
    const centerY =
      $scrollContainer.scrollTop() + $scrollContainer.height() / 2;
