...
export class Mode2Up {
  ...
  jumpToIndex(index) {
    ...
    if (
      index <
      Math.min(this.br.twoPage.currentIndexL, this.br.twoPage.currentIndexR)
    ) {
      this.flipBackToIndex(index);
    } else if (
      index >
      Math.max(this.br.twoPage.currentIndexL, this.br.twoPage.currentIndexR)
    ) {
  ...
  drawLeafs() {
    ...
    this.createPageContainer(indexL)
      .$container.css(this.leftLeafCss)
      .appendTo($twoPageViewEl);
    this.createPageContainer(indexR)
      .$container.css(this.rightLeafCss)
      .appendTo($twoPageViewEl);

    this.displayedIndices = [
      this.br.twoPage.currentIndexL,
  ...
  zoom(direction) {
    ...
    // Get new zoom state
    const reductionFactor = this.br.nextReduce(
      this.br.reduce,
      direction,
      this.br.twoPage.reductionFactors
    );
    if (
    ...
    this.prepareTwoPageView(
      oldCenter.percentageX,
      oldCenter.percentageY,
      drawNewSpread
    );
  }
  ...
  prepareTwoPageView(
    ...
    const targetLeaf = clamp(
      this.br.firstIndex,
      this.br.firstDisplayableIndex(),
      this.br.lastDisplayableIndex()
    );
    ...
    const hasNewDisplayPagesOrDimensions =
      !sameStart || (sameStart && !sameReducer);
    ...
    if (
      this.br.twoPage.totalWidth < this.br.refs.$brContainer.prop("clientWidth")
    ) {
      centerPercentageX = 0.5;
    }
    if (
      this.br.twoPage.totalHeight <
      this.br.refs.$brContainer.prop("clientHeight")
    ) {
    ...
    $(this.br.twoPage.coverDiv)
      .attr("class", "BRbookcover")
      .css(this.spreadCoverCss)
      .appendTo(this.br.refs.$brTwoPageView);

    this.leafEdgeR = document.createElement("div");
    this.leafEdgeR.className = "BRleafEdgeR";
    $(this.leafEdgeR)
      .css(this.leafEdgeRCss)
      .appendTo(this.br.refs.$brTwoPageView);

    this.leafEdgeL = document.createElement("div");
    this.leafEdgeL.className = "BRleafEdgeL";
    $(this.leafEdgeL)
      .css(this.leafEdgeLCss)
      .appendTo(this.br.refs.$brTwoPageView);
  ...
  prunePageContainers() {
    for (const index in this.pageContainers) {
      if (
        index != this.br.twoPage.currentIndexL &&
        index != this.br.twoPage.currentIndexR
      ) {
        $(this.pageContainers[index].$container).remove();
      }
      if (
        index < this.br.twoPage.currentIndexL - 4 ||
        index > this.br.twoPage.currentIndexR + 4
      ) {
  ...
  preparePopUp() {
    ...
    for (const { $leafEdge, jumpIndexForPageX, leftOffset } of leafEdges) {
      ...
      $leafEdge.on("mousemove", (e) => {
        const jumpIndex = clamp(
          jumpIndexForPageX(e.pageX),
          0,
          this.book.getNumLeafs() - 1
        );
        $(this.br.twoPagePopUp).text(
          `View ${this.book.getPageName(jumpIndex)}`
        );
        ...
        $(this.br.twoPagePopUp).css({
          left: `${
            e.pageX -
            this.br.refs.$brContainer.offset().left +
            this.br.refs.$brContainer.scrollLeft() +
            leftOffset()
          }px`,
          top: `${
            e.pageY -
            this.br.refs.$brContainer.offset().top +
            this.br.refs.$brContainer.scrollTop()
          }px`,
        });
      });
    }
  ...
  calculateSpreadSize() {
    ...
    this.br.twoPage.leafEdgeWidthL = this.br.leafEdgeWidth(
      this.br.twoPage.currentIndexL
    );
    this.br.twoPage.leafEdgeWidthR =
      this.br.twoPage.edgeWidth - this.br.twoPage.leafEdgeWidthL;

    // Book cover
    // The width of the book cover div.  The combined width of both pages, twice the width
    // of the book cover internal padding (2*10) and the page edges
    this.br.twoPage.bookCoverDivWidth = this.coverWidth(
      this.br.twoPage.scaledWL + this.br.twoPage.scaledWR
    );
    // The height of the book cover div
    this.br.twoPage.bookCoverDivHeight =
      this.br.twoPage.height + 2 * this.br.twoPage.coverInternalPadding;

    // We calculate the total width and height for the div so that we can make the book
    // spine centered
    const leftGutterOffset = this.gutterOffsetForIndex(firstIndex);
    const leftWidthFromCenter =
      this.br.twoPage.scaledWL -
      leftGutterOffset +
      this.br.twoPage.leafEdgeWidthL;
    const rightWidthFromCenter =
      this.br.twoPage.scaledWR +
      leftGutterOffset +
      this.br.twoPage.leafEdgeWidthR;
    const largestWidthFromCenter = Math.max(
      leftWidthFromCenter,
      rightWidthFromCenter
    );
    this.br.twoPage.totalWidth =
      2 *
      (largestWidthFromCenter +
        this.br.twoPage.coverInternalPadding +
        this.br.twoPage.coverExternalPadding);
    this.br.twoPage.totalHeight =
      this.br.twoPage.height +
      2 *
        (this.br.twoPage.coverInternalPadding +
          this.br.twoPage.coverExternalPadding);
    ...
    this.br.twoPage.gutter =
      this.br.twoPage.middle + this.gutterOffsetForIndex(firstIndex);

    // The left edge of the book cover moves depending on the width of the pages
    // $$$ change to getter
    this.br.twoPage.bookCoverDivLeft =
      this.br.twoPage.gutter -
      this.br.twoPage.scaledWL -
      this.br.twoPage.leafEdgeWidthL -
      this.br.twoPage.coverInternalPadding;
    ...
    // Book spine
    this.br.twoPage.bookSpineDivHeight =
      this.br.twoPage.height + 2 * this.br.twoPage.coverInternalPadding;
    this.br.twoPage.bookSpineDivLeft =
      this.br.twoPage.middle - (this.br.twoPage.bookSpineDivWidth >> 1);
  ...
  getIdealSpreadSize(firstIndex, secondIndex) {
    ...
    const maxLeafEdgeWidth = Math.floor(
      this.br.refs.$brContainer.prop("clientWidth") * 0.1
    );
    ideal.totalLeafEdgeWidth = Math.min(totalLeafEdgeWidth, maxLeafEdgeWidth);

    const widthOutsidePages =
      2 *
        (this.br.twoPage.coverInternalPadding +
          this.br.twoPage.coverExternalPadding) +
      ideal.totalLeafEdgeWidth;
    const heightOutsidePages =
      2 *
      (this.br.twoPage.coverInternalPadding +
        this.br.twoPage.coverExternalPadding);
    ...
    ideal.reduce = Math.round(
      (first.height + second.height) / 2 / ideal.height
    );
  ...
  getSpreadSizeFromReduce(firstIndex, secondIndex, reduce) {
    ...
    const nativeWidth =
      this.book._getPageWidth(firstIndex) +
      this.book._getPageWidth(secondIndex);
    const nativeHeight =
      this.book._getPageHeight(firstIndex) +
      this.book._getPageHeight(secondIndex);
   ...
   */
  getAutofitReduce() {
    const spreadSize = this.getIdealSpreadSize(
      this.br.twoPage.currentIndexL,
      this.br.twoPage.currentIndexR
    );
  ...
  flipBackToIndex(index) {
    ...
    if (
      previousIndices[0] < this.br.firstDisplayableIndex() ||
      previousIndices[1] < this.br.firstDisplayableIndex()
    ) {
      return;
    }
  ...
  flipLeftToRight(newIndexL, newIndexR) {
    ...
    const oldLeafEdgeWidthL = this.br.leafEdgeWidth(
      this.br.twoPage.currentIndexL
    );
    ...
    const top = this.top();
    const gutter =
      this.br.twoPage.middle + this.gutterOffsetForIndex(newIndexL);
    ...
    const right = `${
      ...
      left -
      this.pageContainers[leftLeaf].$container.width() +
    ...
    this.pageContainers[leftLeaf].$container.animate(
      { width: "0px" },
      this.br.flipSpeed,
      "easeInSine",
      () => {
        ...
        this.pageContainers[newIndexR].$container.animate(
          ...
          () => {
            ...
            this.br.textSelectionPlugin?.stopPageFlip(
  ...
  flipFwdToIndex(index) {
    ...
    if (null == index) {
      ...
      const nextPage = this.book
        .getPage(maxDisplayedIndex)
  ...
  flipRightToLeft(newIndexL, newIndexR) {
    ...
    this.pageContainers[this.br.twoPage.currentIndexR].$container.animate(
      { width: "0px" },
      speed,
      "easeInSine",
      () => {
        ...
        this.pageContainers[newIndexL].$container.animate(
          { width: `${newWidthL}px` },
          speed,
          "easeOutSine",
          () => {
  ...
  prepareFlipLeftToRight(prevL, prevR) {
    ...
    this.pageContainers[prevL].$container.css(leftCSS).appendTo($twoPageViewEl);
  ...
  prepareFlipRightToLeft(nextL, nextR) {
    ...
    $(this.pageContainers[nextR].$container)
      .css({
        left: `${gutter}px`,
        top: `${top}px`,
        height: this.br.twoPage.height,
    ...
    $(this.pageContainers[nextL].$container)
      .css({
  ...
  gutter() {
    return (
      this.br.twoPage.middle +
      this.gutterOffsetForIndex(this.br.twoPage.currentIndexL)
    );
  }
  ...
  coverWidth(totalPageWidth) {
    return (
      totalPageWidth +
      this.br.twoPage.edgeWidth +
      2 * this.br.twoPage.coverInternalPadding
    );
  }
  ...
  getViewCenter() {
    ...
    center.percentageX =
      (containerOffset.left -
        viewOffset.left +
        ($brContainer.prop("clientWidth") >> 1)) /
      this.br.twoPage.totalWidth;
    center.percentageY =
      (containerOffset.top -
        viewOffset.top +
        ($brContainer.prop("clientHeight") >> 1)) /
      this.br.twoPage.totalHeight;
  ...
  centerView(percentageX, percentageY) {
    ...
    if (viewWidth < containerClientWidth) {
      ...
      this.br.refs.$brContainer.scrollLeft(
        intoViewX - (containerClientWidth >> 1)
      );
    }
    ...
    if (viewHeight < containerClientHeight) {
      ...
      this.br.refs.$brContainer.scrollTop(
        intoViewY - (containerClientHeight >> 1)
      );
    }
  }
  ...
  flipAreaTop() {
    return Math.floor(
      this.br.twoPage.bookCoverDivTop + this.br.twoPage.coverInternalPadding
    );
  }
  ...
  rightFlipAreaLeft() {
    return Math.floor(
      this.br.twoPage.gutter + this.br.twoPage.scaledWR - this.flipAreaWidth()
    );
  }
  ...
  gutterOffsetForIndex(pindex) {
    ...
    let offset = Math.floor(
      (pindex / this.book.getNumLeafs() - 0.5) * this.br.twoPage.edgeWidth
    );
  ...
  leafEdgeWidth(pindex) {
    ...
    if (
      this.book.getPageSide(pindex) == "L" &&
      this.br.pageProgression != "rl"
    ) {
      return Math.floor(
        (pindex / this.book.getNumLeafs()) * this.br.twoPage.edgeWidth + 0.5
      );
    } else {
      return Math.floor(
        (1 - pindex / this.book.getNumLeafs()) * this.br.twoPage.edgeWidth + 0.5
      );
    }
  }
  ...
  jumpIndexForLeftEdgePageX(pageX) {
    ...
    if ("rl" != this.br.pageProgression) {
      ...
      jumpIndex = clamp(
        ...
        this.br.firstDisplayableIndex(),
        this.br.twoPage.currentIndexL - 2
      );
      return jumpIndex;
    } else {
      jumpIndex =
  ...
  get leafEdgeLCss() {
    return {
      ...
      left: `${
        this.br.twoPage.bookCoverDivLeft + this.br.twoPage.coverInternalPadding
      }px`,
      top: `${
        this.br.twoPage.bookCoverDivTop + this.br.twoPage.coverInternalPadding
      }px`,
  ...
  get spineCss() {
    return {
      ...
      left: `${
        this.br.twoPage.gutter - this.br.twoPage.bookSpineDivWidth / 2
      }px`,
