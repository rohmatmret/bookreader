...
export class Navbar {
      ...
      const $button = this.$nav
        ...
        .on("click", (e) => {
          ...
          this.updateViewModeButton(
            $(e.currentTarget),
            nextViewMode.className,
            nextViewMode.title
          );
          br.switchMode(newViewMode.mode);
        });
      const currentViewModeButton = viewModes.find(
        (m) => m.mode === viewModeOrder[0]
      );
      this.updateViewModeButton(
        $button,
  ...
  init() {
    ...
    this.br.options.controls[
      "bookLeft"
    ].className = `book_left ${bookFlipLeft}`;
    this.br.options.controls[
      "bookRight"
    ].className = `book_right ${bookFlipRight}`;
 ...
 * @return {string}
 */
export function getNavPageNumHtml(
  index,
  numLeafs,
  pageNum,
  pageType,
