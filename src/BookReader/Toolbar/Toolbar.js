...
export class Toolbar {
  ...
  initToolbar(mode, ui) {
    ...
    if (
      !(
        br.canSwitchToMode(br.constMode2up) ||
        br.canSwitchToMode(br.constModeThumb)
      )
    ) {
  ...
  updateToolbarZoom(reduce) {
    ...
    const autofit =
      br.mode == br.constMode2up ? br.twoPage.autofit : br.onePage.autofit;
    /** @type {string} */
    let value;
    if (autofit) {
  ...
  buildShareDiv($shareDiv) {
    ...
    $form.find(".fieldset-embed input").on("change", (event) => {
      ...
      const params = {};
      params.mode = $(
        form.find(".fieldset-embed input[name=pages]:checked")
      ).val();
      ...
      if (br.getEmbedCode) {
        ...
        form
          .find(".BRframeEmbed")
          .val(br.getEmbedCode(frameWidth, frameHeight, params));
      }
    });
